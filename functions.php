...
function getProductFullDesc($product) {
    return $fullDesc = $product->get_description();
}
...
function get_categories_filter_html($args) {
    ...
    // add item all
    $categories_filter_html .= strtr($args['itemTemplate'], array('{categories_filters_content}' => __('All', 'site1'), '{categories_filters_value}' => get_permalink(wc_get_page_id('shop'))));
    // add item featured
    $featuredLink = class_exists('Woocommerce') ? add_query_arg('featured', 'true', get_permalink(wc_get_page_id('shop'))) : '';
    $item = strtr($args['itemTemplate'], array('{categories_filters_content}' => __('Featured', 'site1'), '{categories_filters_value}' => $featuredLink));
